...
class JoystickReader(object):
    ...
    def __init__(self, do_device_discovery=True):
        ...
        self._max_discovery_attempts = 5  # Try for 5 seconds before giving up
    ...
    def _do_device_discovery(self):
        try:
            ...
            elif self._discovery_attempts >= self._max_discovery_attempts:
                # Give up after max attempts to avoid continuous scanning
                logger.info("Device discovery stopped after {} attempts (no devices found)".format(
                    self._discovery_attempts))
                self._discovery_timer.stop()
        except Exception as e:
            logger.error("Error during device discovery: {}".format(e))
            # Stop discovery if there's an error to prevent continuous failures
